...
public:
	TSDecoder_Instance();
	~TSDecoder_Instance();
	int init_audioFilter();
	int configure_audio_filters( const char *afilters, int force_output_format);
	int AudioResample(AVFrame * pAudioDecodeFrame,uint8_t * out_buf);
	...
	int showVideoData(AVFrame* pframe,unsigned long ulpts);
	int showAudioData(AVFrame* pframe,unsigned long ulpts);

	int decode_TimeCalc(unsigned long long &videopts,unsigned long long dts,double stream_timebase,unsigned long long & start_time,int netChatter);
...
private:
	...
	long m_lDisplayPort;
	unsigned char *m_yuvBuff[3]; //3 buffer change
	int m_iCurrentOutBuf;
	pthread_mutex_t m_yuvmutex[3]; //
	...
	avFilter_XH m_avfilter_XH; //video filter xh

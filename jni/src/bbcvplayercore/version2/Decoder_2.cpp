...
const int Recv_Queue_size = 1024*1024*4;   //½ÓÊÕ»º³åÇø´óÐ¡

const int Use_ffmpeg_recv = 0; // 1ÎªÊ¹ÓÃffmpeg½ÓÊÕ£¬0Îª×Ô¼º½ÓÊÕÊý¾Ý¡£	
...
//#ifdef H264_ONLY
#if 0
	if(!this0->m_bDecoderFlag)//false Ö¸¶¨²¥·Å
	{
	...
	sdlParam.audioParam.audioChannels =  pAudioCodecCtx->channels;
	sdlParam.audioParam.audioFormat = pAudioCodecCtx->sample_fmt; //s16
	sdlParam.audioParam.audioFreq = pAudioCodecCtx->sample_rate;
	/*
	av_samples_get_buffer_size(NULL, pAudioCodecCtx->channels,  
	...
	*/
	...
	LOGD("init sdl param: audio: channels=%d,format=%d,sample=%d,size=%d",sdlParam.audioParam.audioChannels,
		sdlParam.audioParam.audioFormat,sdlParam.audioParam.audioFreq,sdlParam.audioParam.audioSamples);
	int iret = this0->init_SDL(sdlParam);
	if(iret >= 0)
		this0->m_bInitSDL = true;
#endif
...
*/
				...
				//			avpicture_deinterlace((AVPicture *)pframe,(AVPicture *)pframe,AV_PIX_FMT_YUV420P,iwidth,iHeight);
						...
						this0->m_pRender->m_renderbuffer = buff;
			...
			else if (pFmt->streams[pkt.stream_index]->codec->codec_type == AVMEDIA_TYPE_AUDIO) 
			{
				...
				if(!this0->m_bGetData){
					...
					av_free_packet(&pkt);   //ÊÓÆµ½âÂë³É¹¦Ö®ºó¿ªÊ¼½âÂëÒôÆµ¡£
					continue;
				}
...
int TSDecoder_Instance::init_SwrConverAudio(AVFrame * pAudioDecodeFrame,uint8_t * out_buf)
{
	...
	int data_size = 0;	
	int ret = 0;  
	int64_t src_ch_layout = AV_CH_LAYOUT_STEREO; //³õÊ¼»¯ÕâÑù¸ù¾Ý²»Í¬ÎÄ¼þ×öµ÷Õû  
	int64_t dst_ch_layout = AV_CH_LAYOUT_STEREO;  // AV_CH_LAYOUT_SURROUND; ; //ÕâÀïÉè¶¨ok  
...
*/
	if (avformat_open_input(&m_pFmt, "", piFmt, NULL) < 0) {
		...
		memset(&systm,0,sizeof(systm));
		GetLocalTime(&systm);
		fLOGD(fpLog,"%2d:%2d:%3d ´ò¿ªÍøÂçÊý¾ÝÊ§°Ü\n",systm.wMinute,systm.wSecond,systm.wMilliseconds);
		fflush(fpLog);
		*/
		return -1;
	} else {
	/*
		memset(&systm,0,sizeof(systm));
		GetLocalTime(&systm);
		fLOGD(fpLog,"%2d:%2d:%3d ´ò¿ªÍøÂçÊý¾Ý³É¹¦\n",systm.wMinute,systm.wSecond,systm.wMilliseconds);
		fflush(fpLog);
		*/
	}
//#else
...
//	LOGD("**********vidoe eum=%d auido enum=%d*******video type=%d audiotype=%d \n",m_vCodetype,m_aCodeType,videoCodeID,audioCodeID);
	...
	memset(&systm,0,sizeof(systm));
	GetLocalTime(&systm);
	fLOGD(fpLog,"%2d:%2d:%3d Ã½ÌåÁ÷Êý¾Ý£¬ÊÓÆµÀàÐÍ£ºH264, ÒôÆµÀàÐÍ mp2 \n",systm.wMinute,systm.wSecond,systm.wMilliseconds);
	fflush(fpLog);
*/
...
//	AVInputFormat *piFmt = NULL;
...

	{
		...
		//·Ö±æÂÊ ÂëÂÊ GOPÖµ  AVCodecContext
...
int TSDecoder_Instance::init_SDL(SDLParam sdlParam)
{
	...
	//render
	LOGD("init sdl video :sw=%d,sh=%d,tw=%d,th=%d",sdlParam.iScreenWidth,sdlParam.iScreenHeight,
		sdlParam.iTextureWidth,sdlParam.iTextureHeight);
...
//#endif	
	...
	LOGD("init sdl video :sw=%d,sh=%d,tw=%d,th=%d",sdlParam.iScreenWidth,sdlParam.iScreenHeight,
		sdlParam.iTextureWidth,sdlParam.iTextureHeight);
	sdlParam.audioParam.audioChannels =  m_pAudioCodecCtx->channels;
	sdlParam.audioParam.audioFormat = m_pAudioCodecCtx->sample_fmt; //s16
	sdlParam.audioParam.audioFreq = m_pAudioCodecCtx->sample_rate;
	/*
	av_samples_get_buffer_size(NULL, pAudioCodecCtx->channels,  
        pAudioCodecCtx->sample_rate,  
        pAudioCodecCtx->sample_fmt, 1);  
...
*/	
	if(m_usedQueue.size() >0)
	{
		...
		*iHeight = tempbuff->iHeight;
		if(video_pts)
			*video_pts = tempbuff->pts;
		//m_ulvideo_pts = tempbuff->pts; //¿ØÖÆÒôÊÓÆµÍ¬²½
...
bool TSDecoder_Instance::set_tsDecoder_stat(bool bstat)
{
	...
	m_bDecoderFlag = bstat; //true Îª×Ô¶¯Æ¥Åä£¬FALSEÎªÖ¸¶¨½âÂë
	return true;
...
/*
			...
			GetLocalTime(&systm);
			fLOGD(fpLog,"%2d:%2d:%3d AutoMatch¿ªÊ¼³õÊ¼»¯ffmpeg\n",systm.wMinute,systm.wSecond,systm.wMilliseconds);
			fflush(fpLog);
*/
	av_register_all();
	avformat_network_init();
...
/*
	...
	{/*
		memset(&systm,0,sizeof(systm));
		GetLocalTime(&systm);
		fLOGD(fpLog,"%2d:%2d:%3d ´ò¿ªÍøÂçÊý¾ÝÐÅÏ¢\n",systm.wMinute,systm.wSecond,systm.wMilliseconds);
		fflush(fpLog);*/
		return -1;
	}
		...
		case CODEC_ID_H264:
			{
				...
				fLOGD(fpLog,"%2d:%2d:%3d ÊÓÆµÀàÐÍ£ºH264\n",systm.wMinute,systm.wSecond,systm.wMilliseconds);
				fflush(fpLog);
				break;
			}
		case CODEC_ID_MPEG2VIDEO:
			{
				...
				fLOGD(fpLog,"%2d:%2d:%3d ÊÓÆµÀàÐÍ£ºMPEG2VIDEO\n",systm.wMinute,systm.wSecond,systm.wMilliseconds);
				fflush(fpLog);
				break;
			}
		case CODEC_ID_MPEG4:
			{
				...
				fLOGD(fpLog,"%2d:%2d:%3d ÊÓÆµÀàÐÍ£ºMPEG4\n",systm.wMinute,systm.wSecond,systm.wMilliseconds);
				fflush(fpLog);
				break;
			}
		case CODEC_ID_H263:
			{
				...
				fLOGD(fpLog,"%2d:%2d:%3d ÊÓÆµÀàÐÍ£ºH263\n",systm.wMinute,systm.wSecond,systm.wMilliseconds);
				fflush(fpLog);
				break;
			}
		default:
	...
	case CODEC_ID_MP2:
		{
			...
			fLOGD(fpLog,"%2d:%2d:%3d ÒôÆµÀàÐÍ£ºMP2\n",systm.wMinute,systm.wSecond,systm.wMilliseconds);
			fflush(fpLog);
			break;
		}
	case CODEC_ID_MP3:
		{
			...
			fLOGD(fpLog,"%2d:%2d:%3d ÒôÆµÀàÐÍ£ºMP3\n",systm.wMinute,systm.wSecond,systm.wMilliseconds);
			fflush(fpLog);
			break;
		}
	case CODEC_ID_AAC:
		{
			...
			fLOGD(fpLog,"%2d:%2d:%3d ÒôÆµÀàÐÍ£ºAAC\n",systm.wMinute,systm.wSecond,systm.wMilliseconds);
			fflush(fpLog);
			break;
		}
	case CODEC_ID_AC3:
		{
			...
			fLOGD(fpLog,"%2d:%2d:%3d ÒôÆµÀàÐÍ£ºAC3\n",systm.wMinute,systm.wSecond,systm.wMilliseconds);
			fflush(fpLog);
			break;
		}
	default:

...
#define LOGD(...) if(LOG_LEVEL <= 1) __android_log_print( ANDROID_LOG_WARN, LOG_TAG, __VA_ARGS__ )
#define LOGE(...) if(LOG_LEVEL <=2)  __android_log_print( ANDROID_LOG_ERROR, LOG_TAG, __VA_ARGS__ )

#else
	#define LOGD(...) if(LOG_LEVEL <= 1)  printf( __VA_ARGS__ )
	#define LOGE(...) if(LOG_LEVEL <= 2)  printf( __VA_ARGS__ )

...
{
	memset(&m_sdlparam,0,sizeof(m_sdlparam));
	memcpy(&m_sdlparam,&sdlParam,sizeof(m_sdlparam));
 ...
 //       m_iScreenWidth, m_iScreenHeight,SDL_WINDOW_OPENGL|SDL_WINDOW_RESIZABLE);  
	...
	LOGE("yyd info create window success");
...
{
	if(buffer == NULL||m_bstop)
	{
		LOGE("yyd info bbcvrender render failed buffer=null  textwidth=%d\n",m_iTextureWidth);
		return -1;
	}	
...
int BBCVRender::InitAudioPlayer(AudioParam pm)
{
	...
	LOGE("yyd info  audio init sdl success \n");
